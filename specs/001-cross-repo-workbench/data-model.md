...
8. **Every session's `last_activity_at` is monotonically non-decreasing.** The mechanism is a **writer-side clamp**, not a paired monotonic clock: every update computes `new_value = max(current_row.last_activity_at, candidate_timestamp)` inside `SessionService::touch_activity` before the `UPDATE` statement lands. Wall-clock backward jumps (NTP correction, laptop sleep-wake, timezone change) are therefore swallowed silently — `last_activity_at` never rewinds, even if the OS clock does. The same clamp discipline applies to `notes.updated_at` and `reminders.done_at` (enforced in their respective services). `created_at` columns are written once and never updated, so they need no clamp. **Age for the stale-reminder indicator (FR-031) and for idle detection is computed as `max(Duration::ZERO, now_utc - created_at)` at read time** — this means age is accurate to the resolution of the wall clock but is not tracked against a separate `std::time::Instant`. Consequence: under a large backward clock jump, all open reminders will briefly appear younger than they really are, but none will appear older or rewind. A unit test in `session/mod.rs` seeds a row with `last_activity_at = now()`, passes in an earlier candidate timestamp, and asserts the stored value is unchanged — this is the enforcement gate for the clamp.
