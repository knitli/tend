...
- **FR-003**: Each session entry MUST display a live status indicating at minimum: working, idle, and needs-input. Additional states (e.g., error, finished) MAY be shown.
...
- **FR-008**: The workbench MUST handle sessions that are started outside of it (e.g., manually launched) so they appear in the unified view without requiring the user to re-launch them from the workbench.
...
- **FR-014**: The workbench MUST preserve the identity and project mapping of a session across its lifetime, even if its underlying window/pane is moved, renamed, or reopened.
...
- **FR-020**: On relaunch, the workbench MUST reattach to any still-running sessions and their companion terminals, and MUST clearly mark as "not running" any sessions that did not survive the restart.
...
- **Session**: A running agent instance tied to exactly one project. Has a label, a status (working / idle / needs-input / ended / error), a current activity summary, timestamps for last activity and last user attention, a reference to its underlying window/pane so the workbench can focus it, and a reference to a paired Companion Terminal.
...
- **SC-006**: Daily setup time to get back to a multi-project working state drops by at least 50% compared to manually reopening terminals and sessions, as measured by user self-report or timed task.
- **SC-007**: Over a representative working session, the user does not miss a "needs input" event (i.e., no session sits in "needs input" for longer than the user's configured quiet-hours policy allows before the user notices it).
- **SC-008**: When the workbench is closed and reopened without a reboot, at least 95% of still-running sessions and their companion terminals are reattached and displayed correctly without the user having to intervene.
- **SC-009**: First-time users can register a project and see its first agent session appear in the workbench — with its companion terminal paired automatically — in under 3 minutes without reading documentation.
- **SC-010**: A user with scratchpads across multiple projects can answer "what am I in the middle of, everywhere?" by opening the cross-project overview in under 15 seconds, without opening any individual session.
- **SC-011**: Adding a note or reminder to the current project takes no more than two actions from anywhere in the workbench.
- **SC-012**: Open reminders persist indefinitely across workbench restarts and system reboots until the user marks them done; zero open reminders are lost due to lifecycle events (session ending, reboot, workbench restart).
