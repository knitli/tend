...
- [ ] T001 Create Cargo workspace root `Cargo.toml` at `/home/knitli/agentui/Cargo.toml` with members `["protocol", "src-tauri", "cli"]` and shared `[workspace.dependencies]` for `tokio`, `sqlx`, `serde`, `serde_json`, `tracing`, `thiserror`, `anyhow`
- [ ] T002 Create `src-tauri/Cargo.toml` with Tauri 2.x, `portable-pty`, `sqlx` (sqlite, runtime-tokio-rustls, macros), `tokio` (full), `notify`, `sysinfo`, `tracing`, `tracing-subscriber`, `serde`, `serde_json`, `thiserror`, `anyhow`, `dirs` (for XDG), `tauri-plugin-notification`, **`agentui-protocol = { path = "../protocol" }` — no path-dep into `cli/`, no local redefinition of wire types**
- [ ] T003 Create `cli/Cargo.toml` with `clap` (derive), `portable-pty`, `tokio` (rt, io-util, net, macros), `serde`, `serde_json`, `anyhow`, `dirs`, **`agentui-protocol = { path = "../protocol" }` — no path-dep into `src-tauri/`, no local redefinition of wire types**
- [ ] T003b [P] Create `protocol/Cargo.toml` as a library crate named `agentui-protocol` with minimal deps (`serde` with `derive`, `serde_json`, `thiserror`) — **no tokio, no sqlx, no tauri**, this crate must stay build-cheap and reuse-safe. Set `publish = false` for v1.
