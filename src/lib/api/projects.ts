...
export async function projectRegister(
  opts: { path: string; displayName?: string },
...
export async function projectUpdate(
  opts: { id: number; displayName?: string; settings?: ProjectSettings },
): Promise<{ project: Project }> {
  return invoke<{ project: Project }>('project_update', {
    args: { id: opts.id, display_name: opts.displayName, settings: opts.settings },
  });
}
...
export async function projectArchive(
  ...
  await invoke<Record<string, never>>('project_archive', { args: { id: opts.id } });
}
...
export async function projectUnarchive(
  ...
  return invoke<{ project: Project }>('project_unarchive', { args: { id: opts.id } });
}
